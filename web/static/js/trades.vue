  ...
  <div class="vue-root">
      ...
      <div class="card-header">
        ...
        <span class="text-muted float-right"
          ><transition name="slide-fade" mode="out-in"
            ><div :key="positionsUpdatedAt">{{ positionsUpdatedAt }}</div></transition
          ></span
        >
      </div>
      <!-- /.card-header -->
          ...
          <table class="table table-bordered table-sm table-hover">
            ...
            <tbody>
              <tr v-for="position in positions" :key="`${position.exchange}-${position.position.symbol}`">
                <td>
                  <img
                    :src="`/img/exchanges/${position.exchange}.png`"
                ...
                <td>
                  <a target="blank" :href="'/tradingview/' + position.exchange + ':' + position.position.symbol">{{
                    position.position.symbol
                  }}</a>
                </td>
                <td
                  v-bind:class="{
                    'text-success': position.position.amount > 0,
                    'text-danger': position.position.amount < 0,
                  }"
                >
                ...
                <td>
                  <span
                    v-if="typeof position.position.profit !== 'undefined'"
                    v-bind:class="{
                      'text-success': position.position.profit >= 0,
                      'text-danger': position.position.profit < 0,
                    }"
                  >
                ...
                <td>
                  <i
                    v-if="position.position.side === 'short'"
                    class="fas fa-chevron-circle-down text-danger"
                    title="short"
                  ></i>
                  <i
                    v-if="position.position.side === 'long'"
                    class="fas fa-chevron-circle-up text-success"
                    title="long"
                  ></i>
                </td>
    ...
    <div class="card">
      <div class="card-header">
        ...
        <span class="text-muted float-right"
          ><transition name="slide-fade" mode="out-in"
            ><div :key="positionsUpdatedAt">{{ ordersUpdatedAt }}</div></transition
          ></span
        >
      </div>
      <div class="card-body">
          ...
          <table class="table table-bordered table-sm table-hover">
            ...
            <tbody>
              <tr v-for="order in orders" :key="`${order.exchange}-${order.order.symbol}-${order.order.id}`">
                <td>
                  <img
                    :src="`/img/exchanges/${order.exchange}.png`"
                    :alt="order.exchange"
                ...
                <td>
                  <a target="blank" :href="'/tradingview/' + order.exchange + ':' + order.order.symbol">{{
                    order.order.symbol
                  }}</a>
                </td>
                ...
                <td>
                  {{ order.order.price
                  }}<span class="text-muted" v-if="order.percent_to_price" title="Percent to current price">
                    {{ order.percent_to_price | round(1) }} %</span
                  >
                </td>
                <td v-bind:class="{ 'text-success': order.order.amount > 0, 'text-danger': order.order.amount < 0 }">
                  {{ order.order.amount }}
                </td>
                ...
                <td>
                  <a title="cancel" :href="'/order/' + order.exchange + '/' + order.order.id"
